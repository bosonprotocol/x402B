...
2. Negotiates the fulfilment channel.
3. Decides between `createOfferAndCommit` (commit now, redeem later) and `createOfferCommitAndRedeem` (commit and redeem in one tx, regardless of when the resource is delivered) based on the buyer's policy and what `actions.next[]` allows. This is independent of the chosen fulfilment channel.
