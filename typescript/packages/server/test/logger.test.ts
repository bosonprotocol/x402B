...
describe("facilitator client logger wiring", () => {
  it("logs a warn event on network error", async () => {
    ...
    const client = createFacilitatorClient({
      ...
      fetch: stubFetch,
      logger: rec.logger,
      // Disable retries so the test isn't pinned to the 600ms backoff
      // window. The logger emit point is per-attempt; one attempt is
      // enough to exercise the warn event.
