...
export function createX402bServer(config: X402bServerConfig): X402bServer {
  ...
  const recovery: RecoveryApi = {
    async list() {
      // Snapshot at call time so callers iterate a stable view even if
      // a handler concurrently mutates the store.
      return [...fulfillmentRecoveryStore.values()];
    },
    async replay(exchangeId) {
      const entry = fulfillmentRecoveryStore.get(exchangeId);
      if (entry === undefined) {
      ...
      if (channel === undefined) {
        ...
        fulfillmentRecoveryStore.set(exchangeId, { ...entry, error: reason });
        return { ok: false, reason };
      }
      try {
        await channel.onCommit(exchangeId, entry.data);
        fulfillmentRecoveryStore.delete(exchangeId);
        return { ok: true };
      } catch (e) {
