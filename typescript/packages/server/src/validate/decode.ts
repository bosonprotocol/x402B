...
export function decodeXPaymentHeader(header: string | undefined | null): DecodeXPaymentResult {
  ...
  // to recover the original UTF-8 (atomic Flow B's `fulfillment.data`
  // can carry non-ASCII bytes — names, addresses, free-form notes —
  // and so can other buyer-supplied inner fields).
