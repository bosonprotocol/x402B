...
Rather than fork that complexity into per-seller bespoke flows, x402b standardizes a `DeliveryTransport` interface. The 402 advertises which transports the seller supports; the buyer picks one and attaches transport-specific data alongside the payment payload (or out-of-band, depending on transport).
...
"delivery": {
  ...
  "options": [
    {
      "id": "<transport-id>",        // see registry below
      "schema": <JSON Schema>,        // shape of the data the buyer must attach
      "metadata": { /* per-transport hints, e.g. server's webhook url */ }
    }
  ]
}
```
...
```jsonc
"delivery": {
  "option": "<chosen transport id>",
  "data":   { /* validates against the chosen option's schema */ }
}
```

For transports where data is collected post-commit (e.g. `widget`), `delivery.data` is `null` in the X-PAYMENT and the actual collection happens between commit and redeem via the transport's own channel.

## `DeliveryTransport` interface (TypeScript)

```ts
export interface DeliveryTransport<TServerCfg = unknown, TBuyerData = unknown> {
  /** Stable identifier used in the wire format. */
  readonly id: string;

  /** JSON Schema describing what the buyer must put in `delivery.data`. */
  ...
  onRedeem(exchangeId: string): Promise<DeliveryResult>;
...
## Initial transport registry

| `id` | Use case | Buyer-data schema | Notes |
|---|---|---|---|
| `atomic-http` | Resource returned in same HTTP response | `null` | Server `onRedeem` returns the body. Composes naturally with Flow B's atomic commit-and-redeem (the resource is ready), but Flow B does **not** require this transport — the redeem state transition can be atomic while delivery itself is async. |
...
| `mcp` | AI-agent buyer drives a server-exposed MCP tool | `{ mcpEndpoint?: string }` | The seller's MCP exposes a `submit_delivery_data(exchangeId, ...)` tool. The buyer's agent calls it post-commit. |

The registry is open: third parties can ship additional transports as `@bosonprotocol/x402-delivery-<id>` packages and register them with the SDK at startup.

## When data is collected

| Transport | Collected at | Why |
|---|---|---|
| `atomic-http` | n/a | No data. |
| `email`, `xmtp`, `webhook`, `ipfs-pointer` | At commit (in X-PAYMENT) | Lightweight, no side-channel needed. |
| `widget`, `mcp` | Post-commit, via the transport's channel | Heavier UI / agent-driven; not a fit for header-sized data. |

Servers SHOULD advertise at least one "in-payload" transport (email/xmtp/webhook) so headless agents can complete in a single round trip.

## Privacy considerations

- `delivery.data` is stored server-side keyed by exchangeId. Servers MUST NOT include it in any unauthenticated channel.
...
1. If `requirements.delivery.required === true`, `payload.delivery.option` MUST be present and MUST match a registered transport.
2. The chosen option's `buyerDataSchema` MUST validate `payload.delivery.data` (or `null` if schema is `null`).
3. The server-side instance of the transport MUST be configured (at boot, not per-request).
4. After commit acceptance, server calls `transport.onCommit(exchangeId, data)` BEFORE returning 200.

## Client-side helper

```ts
import { negotiateDelivery } from "@bosonprotocol/x402-delivery";

const choice = await negotiateDelivery(requirements.delivery.options, {
...
`negotiateDelivery` returns the first option that (a) the client supports, (b) the client has data for (or can collect), and (c) is acceptable to the agent's policy.
