...
| Package | Purpose |
|---|---|
| [`x402-core`](https://www.npmjs.com/package/@bosonprotocol/x402-core) | `escrow` scheme JSON schemas + TypeScript types; EIP-712 builders for FullOffer (protocol domain), the protocol meta-tx envelope, and the four BPIP-12 token-auth strategies (ERC-3009 ReceiveWithAuthorization, EIP-2612 Permit, Permit2, plain approve); exchange state machine model. |
| [`x402-evm`](https://www.npmjs.com/package/@bosonprotocol/x402-evm) | EVM-specific implementation. Calldata builders for `ExchangeCommitFacet.createOfferAndCommit` (deferred), `OrchestrationHandlerFacet2.createOfferCommitAndRedeem` (atomic on-chain redeem), and the `MetaTransactionsHandlerFacet.executeMetaTransactionWithTokenTransferAuthorization` envelope that carries them. Wraps `@bosonprotocol/core-sdk`. |
| [`x402-server`](https://www.npmjs.com/package/@bosonprotocol/x402-server) | Framework-agnostic resource server. 402 builder, FullOffer signer wrapper, fulfillment negotiator, `nextActions` emitter, post-redeem endpoint set. Express adapter below (further adapters planned). |
| [`x402-server-express`](https://www.npmjs.com/package/@bosonprotocol/x402-server-express) | Express adapter for `x402-server` — middleware + mountable router that wires the 402 builder and post-redeem endpoint set into an Express app. |
| [`x402-client`](https://www.npmjs.com/package/@bosonprotocol/x402-client) | Framework-agnostic client. Interceptor that parses the 402, picks a fulfillment channel option and a token-auth strategy, signs the meta-tx + token authorization(s), retries, then drives post-redeem actions through whichever channel is preferred. `fetch` adapter below (further adapters planned). |
| [`x402-client-fetch`](https://www.npmjs.com/package/@bosonprotocol/x402-client-fetch) | Native-`fetch` adapter for `x402-client` — wraps `fetch` to intercept 402 escrow-scheme responses and retry with the generated `X-PAYMENT` header. |
| [`x402-client-browser`](https://www.npmjs.com/package/@bosonprotocol/x402-client-browser) | Browser-environment adapters for the buyer SDK — `signerFromWalletClient` (wraps a viem `WalletClient`) and `signerFromEip1193` (wraps a raw EIP-1193 provider such as `window.ethereum`); re-exports the full `x402-client` surface so a single install covers the common browser-buyer case. |
...
## Implementation specification document map
